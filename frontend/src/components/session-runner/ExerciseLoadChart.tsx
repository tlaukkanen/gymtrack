...
interface ExerciseLoadChartProps {
  points?: WorkoutSessionExerciseProgressPointDto[]
  currentSessionExerciseId: string
  isLoading?: boolean
}

type ExerciseChartDatum = WorkoutSessionExerciseProgressPointDto & {
  label: string
}

const ExerciseLoadChart = ({ points, currentSessionExerciseId, isLoading = false }: ExerciseLoadChartProps) => {
  const data = useMemo<ExerciseChartDatum[]>(() => {
    if (!points) return []
  ...
  const renderDot = (props: any) => {
    const { cx, cy, payload } = props as { cx?: number; cy?: number; payload?: ExerciseChartDatum }
    if (!cx || !cy || !payload) return null
    if (payload.sessionExerciseId === currentSessionExerciseId) {
  ...
  return (
    <Card>
      ...
      <div style={{ height: 220 }}>
        {isLoading ? (
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Loading exercise history…</p>
        ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ left: 8, right: 12, top: 10, bottom: 0 }}>
              ...
              <XAxis dataKey="label" tick={{ fill: 'var(--text-muted)', fontSize: 12 }} stroke="var(--text-muted)" />
              <YAxis
                tickFormatter={(value) => `${weightFormatter.format(value as number)} kg`}
              ...
              <Tooltip
                ...
                formatter={(value: number) => [`${weightFormatter.format(value)} kg`, 'Exercise load']}
              />
              <Line
                type="monotone"
